...
3. **语义安全检测**（以下情况标记为 HIGH 风险，放入 risk_warnings）：
   - **SQL注入风险**：修复代码中使用 f-string 或字符串拼接构建 SQL 查询并执行，例如 `f"SELECT * FROM users WHERE id = {user_input}"`
   - **硬编码敏感信息**：修复代码将 API 密钥、密码、Token 等直接赋值为字符串常量
   - **逻辑破坏**：修复代码中破坏原有数据结构的完整性，如将列表/字典变量整体置为 None、用字符串索引列表、删除必要的方法调用等
