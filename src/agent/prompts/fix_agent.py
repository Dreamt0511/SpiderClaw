...
## 核心约束
1. 必须修复 error_locations 中列出的**每一个**错误，遗漏任何一个都会被拒绝。不做其他任何修改
2. 禁止改变函数签名（参数列表或返回值类型），除非修复逻辑确实需要
3. 禁止返回自然语言解释，只输出 JSON
4. 禁止编造不存在的文件路径
5. fix_description 用简洁中文，每个修复点一行

## 异常处理修复指导
1. ✅ 推荐：在函数入口添加参数校验，提前抛出更明确的异常（如`ValueError`）替代隐含的运行时异常
2. ✅ 允许：如果测试用例没有异常捕获逻辑且明确期望正常返回，可以修改异常处理行为返回合理值
3. ✅ 允许：优化现有异常处理逻辑，完善错误日志记录
4. ❌ 禁止：无理由地吞掉异常并返回0、None等无意义的默认值导致静默失败
