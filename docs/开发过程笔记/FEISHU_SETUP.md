...
3. **等待授权完成**
   ...
   请将以上凭据配置到 src/config/agent-config.yaml 中
   ============================================================
   ```
...
4. **配置自动写入**
   ...
   ```
   🔧 步骤 2/3: 更新配置文件
   ✅ 配置已保存到: src/config/agent-config.yaml
   ```
...
5. **配置完成**
   ...
   💡 后续配置:
      1. 在 src/config/agent-config.yaml 中配置需要通知的用户/群组ID
      2. 重启服务后即可自动发送飞书通知
...
配置完成后，你可以在 `src/config/agent-config.yaml` 中进一步配置：
