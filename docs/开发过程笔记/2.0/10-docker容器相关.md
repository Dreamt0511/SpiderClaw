...
# ── 手动触发测试，全部测试文件 ──
docker exec -it biz-server bash -c "cd /opt/biz-app && python3 -m pytest app/tests/ -v --tb=long"

#测试test_user_service.py文件
docker exec -it biz-server bash -c "cd /opt/biz-app && python3 -m pytest app/tests/test_user_service.py -v --tb=long"

#测试test_calculator.py文件
docker exec -it biz-server bash -c "cd /opt/biz-app && python3 -m pytest app/tests/test_calculator.py -v --tb=long"
